...
namespace Ceemas.DataAccess.Core.SqlStatementBuilder.Builder
{
    ...
    public class SelectStatementBuilderTest
    {
        ...
        [TestMethod]
        public void SelectGroupByCountTest()
        {
            var statementBuilder = SqlStatement.Select(select =>
            {
                select.From("SCHEMANAME", "TABLENAME", "ALIASNAME", columns => columns
                ...
                .GroupBy(column => column.TableColumn("ALIASNAME", "COLUMN1"));
            });

            var statement = statementBuilder.ToString();
            Assert.AreEqual("SELECT ALIASNAME.COLUMN1 AS COLUMNALIAS1, COUNT(*) AS COUNTCOLUMNNAME FROM SCHEMANAME.ALIASNAME ALIASNAME GROUP BY ALIASNAME.COLUMN1", statement);
        }
    }
}
